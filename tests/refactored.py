...
class Drawable:
    ...
    def _filled_circle_optimized(
        ...
        Parameters:
        - image: NumPy image array (dtype=np.uint8).
        - center: (y, x) coordinates of the circle.
        - radius: Circle radius.
        - color: Fill color (RGB or RGBA tuple).
        - outline_color: Color for the outline (default: None).
        - outline_width: Width of the outline.

        Returns:
        - Modified image with the circle.
        """
        x, y = center
        h, w = image.shape[:2]

        # Convert color to match image dtype
        color_np = np.array(color, dtype=image.dtype)

        if outline_color is not None:
            outline_color_np = np.array(outline_color, dtype=image.dtype)
        else:
    ...
    async def async_draw_obstacles(image: np.ndarray, obstacle_info_list,
        ...
        Parameters:
        - image: NumPy image array (modified in-place).
        - obstacle_info_list: List of obstacle dictionaries containing "x" and "y".
        - color: Color of obstacles (RGB or RGBA).

        Returns:
        - Modified image with filled circles drawn in parallel.
        """

        def extract_centers(obstacle_info_list):
            """
            Extracts (x, y) centers from obstacle_info_list and converts them into a NumPy array.

            Parameters:
            - obstacle_info_list: List of dictionaries with "points" containing "x" and "y".
