...
class PerformanceProfiler:
    ...
    def __init__(
        self,
        enable_memory_profiling: bool = ENABLE_MEMORY_PROFILING,
        enable_cpu_profiling: bool = ENABLE_CPU_TIMING,
    ):
    ...
    def analyze_memory_usage(self) -> None:
        ...
        if len(self.memory_stats) >= 2:
            ...
            print(
                f"   End RSS:   {end_rss:.1f}MB ({'+' if end_rss > start_rss else ''}{end_rss - start_rss:.1f}MB from start)"
            )
    ...
    def analyze_timing_patterns(self) -> None:
        ...
        for operation, durations in timing_groups.items():
            ...
            print(
                f"   {operation:20s} | Avg: {avg_duration:6.1f}ms | Min: {min_duration:6.1f}ms | Max: {max_duration:6.1f}ms | Count: {len(durations)}"
            )
...
class TestImageHandler:
    ...
    def set_up(self):
        ...
        # Load the test.json file rand_converted
    ...
    async def test_image_handler(self, frame_queue=None):
        ...
        device_info = {
            ...
            "color_material_wood": [
                ...
                40,
                40,
            ],  # Bright RED for wood grain lines (testing)
            "alpha_material_wood": 25,  # High alpha for very visible wood grain
            "color_material_tile": [
                40,
                40,
                40,
        ...
        shared.mop_mode = True  # Enable mop mode to test path width adjustment
        ...
        for i in range(FRAME_COUNT):
            ...
            if self.profiler and ENABLE_MEMORY_PROFILING:
                if SNAPSHOT_EVERY_FRAME or iteration in (1, FRAME_COUNT):
                    self.profiler.take_memory_snapshot(
                        f"Before Image Generation #{iteration}"
                    )
            ...
            self.image, data = await handler.async_get_image(
                self.test_data, bytes_format=True
            )
            _LOGGER.debug(
                f"Image generated: {data['image']['size']}, {data['attributes']}"
            )
            # Check if image generation was successful
            if data["image"]["binary"] is None:
            ...
            if SAVE_FIRST_FRAME and iteration == 1 and self.image:
                try:
                    ...
                    _LOGGER.info(
                        "✅ Saved first frame to /tmp/test_frame_1_with_zones.png"
                    )
                except Exception as e:
                    _LOGGER.warning(f"Failed to save frame: {e}")

            # Notify main thread with the new frame to display
            if frame_queue is not None:
                try:
                    frame_queue.put(
                        self.image
                    )  # blocks if previous frame not yet displayed
                except Exception as e:
                    _LOGGER.warning(f"Failed to enqueue frame for display: {e}")
            ...
            if proc and cpu0 and cpu1:
                ...
                _LOGGER.info(
                    f"CPU/frame: {cpu_used:.3f}s | core-util: {core_util:.1f}% of one core"
                )

            # Optional profiler bookkeeping
            if self.profiler:
                if ENABLE_MEMORY_PROFILING and (
                    SNAPSHOT_EVERY_FRAME or iteration in (1, FRAME_COUNT)
                ):
                    self.profiler.take_memory_snapshot(
                        f"After Image Generation #{iteration}"
                    )
                self.profiler.time_operation(
                    f"Image #{iteration}", start_time, end_time
                )
                if cpu_used is not None:
                    self.profiler.timing_stats.append(
                        {
                            "label": f"CPU #{iteration}",
                            "duration_ms": cpu_ms,
        ...
        _LOGGER.info(f"Calibration_data (shared): {shared.attr_calibration_points}")
        _LOGGER.info(f"Room Properties (shared): {shared.map_rooms}")

        # Robot in room timing (using existing robot_pos)
        rp = handler.robot_pos or {}
        rx, ry, ra = rp.get("x"), rp.get("y"), rp.get("angle")
        t4 = time.time()
        _ = await handler.imd.async_get_robot_in_room(robot_y=ry, robot_x=rx, angle=ra)
        t5 = time.time()
        if self.profiler:
            self.profiler.time_operation("RobotRoom", t4, t5)

        _LOGGER.debug(f"Calibration_data (shared): {shared.attr_calibration_points}")
        _LOGGER.debug(f"PIL image size: {self.image.size}")
        _LOGGER.debug(f"Room Properties (shared): {shared.map_rooms}")
        count = store.get_rooms_count()
        _LOGGER.debug(f"Room Store Properties: {count}")
        rooms = store.get_rooms()
        instaces = RoomStore.get_all_instances()
        _LOGGER.debug(f"Room Store Rooms {instaces}: {rooms}")
        ...
        for i, active in enumerate(handler.active_zones or []):
            if i < len(room_keys):
                _LOGGER.info(
                    f"Position {i}: Segment ID '{room_keys[i]}' ({rooms[room_keys[i]]['name']}) = active: {bool(active)}"
                )
            else:
        ...
        _LOGGER.info(f"Calibration Data (shared): {shared.attr_calibration_points}")
        _LOGGER.info(f"Current room (shared): {shared.current_room}")
        # Keep robot position for robot-in-room check
        ...
        result = await handler.imd.async_get_robot_in_room(
            robot_y=robot_y, robot_x=robot_x, angle=robot_angle
        )
        ...
        _LOGGER.info(
            f"Zoom conditions: zoom={handler.imd.img_h.zooming}, vacuum_state={handler.shared.vacuum_state}, image_auto_zoom={handler.shared.image_auto_zoom}"
        )
...
def __main__():
    # Prepare test and disable CPU profiling inside custom profiler to avoid conflicts
    ...
    # Helper to fetch per-thread CPU time (user + system) in seconds
    def _thread_cpu_seconds(tid: int) -> float | None:
        try:
            for th in psutil.Process().threads():
                if th.id == tid:
                    return float(
                        getattr(th, "user_time", 0.0) + getattr(th, "system_time", 0.0)
                    )
        except Exception:
            pass
        return None

    results = {"camera_cpu": None, "camera_tid": None}

    # Worker function to run image processing on a separate thread (camera_processing)
    def camera_processing():
        try:
            ...
            # Record this thread's TID and CPU start time
            my_tid = threading.get_native_id()
            results["camera_tid"] = my_tid
            cam_cpu_start = _thread_cpu_seconds(my_tid)

            try:
                loop.run_until_complete(
                    test.test_image_handler(frame_queue=frame_queue)
                )
            finally:
                loop.close()

            # Record camera thread CPU end time
            cam_cpu_end = _thread_cpu_seconds(my_tid)
            if cam_cpu_start is not None and cam_cpu_end is not None:
                results["camera_cpu"] = max(0.0, cam_cpu_end - cam_cpu_start)

            if test.profiler:
                test.profiler.take_memory_snapshot("Test Complete")
        except Exception as exc:  # Avoid crashing silently; keep it minimal
            _LOGGER.warning(f"camera_processing thread failed: {exc}")
        finally:
            # Always signal completion to main thread to avoid deadlocks
            try:
                frame_queue.put(None, timeout=1)
            except Exception:
                pass

    # Start camera_processing thread
    # Queue to transfer frames from camera_processing to main thread for display
    frame_queue = queue.Queue(maxsize=1)

    cam_thread = threading.Thread(target=camera_processing, name="camera_processing")
    cam_thread.start()

    # Main thread: receive frames and display each image without artificial CPU load
    ...
    while True:
        try:
            item = frame_queue.get(timeout=30)  # avoid indefinite blocking
        except queue.Empty:
            print(
                "Timeout waiting for frame from camera_processing; exiting display loop."
            )
            break
        if item is None:
            break
    ...
    # Wait for camera_processing to finish
    cam_thread.join()

    # Report per-thread CPU usage (aggregated over frames)
    if frames_shown > 0:
        print(
            f"Main thread displayed {frames_shown} frame(s). CPU time spent in display: {main_cpu_total:.3f}s"
        )
    if results["camera_cpu"] is not None and results["camera_tid"] is not None:
        cam_cpu = results["camera_cpu"]
        print(
            f"camera_processing CPU (tid={results['camera_tid']}): {cam_cpu:.3f}s (total)"
        )
        if frames_shown > 0 and main_cpu_total > 0 and cam_cpu > 0:
            ratio_main_to_cam = main_cpu_total / cam_cpu
            ratio_cam_to_main = cam_cpu / main_cpu_total
            print(f"Main/Camera CPU ratio: {ratio_main_to_cam:.2f}x")
            print(f"Camera/Main CPU ratio: {ratio_cam_to_main:.2f}x")

    # Generate comprehensive profiling report and adjusted stats
    def _avg(values: list[float]) -> float:
        return sum(values) / max(len(values), 1)

    if hasattr(test, "wall_times_ms") and test.wall_times_ms:
        steady_wall = (
            test.wall_times_ms[1:]
            if len(test.wall_times_ms) > 1
            else test.wall_times_ms
        )
        steady_cpu = (
            test.cpu_times_ms[1:] if len(test.cpu_times_ms) > 1 else test.cpu_times_ms
        )
        print("\n=== ADJUSTED (steady-state, excl. warm-up) ===")
        if steady_wall:
            print(
                f"Image avg (ms): {_avg(steady_wall):.1f} | min: {min(steady_wall):.1f} | max: {max(steady_wall):.1f} | n={len(steady_wall)}"
            )
        if steady_cpu:
            print(
                f"CPU avg (ms):   {_avg(steady_cpu):.1f} | min: {min(steady_cpu):.1f} | max: {max(steady_cpu):.1f} | n={len(steady_cpu)}"
            )
