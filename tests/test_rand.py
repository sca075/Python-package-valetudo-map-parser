    ...
    def setUp(self):
        ...
        self.test_files = [
            (
                ...
                "FIRST FILE - Single room, vacuum at dock",
            ),
            (
            ...
            (
                "map_data_20250728_194519.bin",
                "FIRST FILE - Single room, vacuum at dock",
            ),
            (
                "map_data_20250728_204538.bin",
                "FIRST FILE - Single room, vacuum at dock",
            ),
            (
                "map_data_20250728_204552.bin",
                "FIRST FILE - Single room, vacuum at dock",
            ),
            (
                "map_data_20250729_084141.bin",
                "LAST FILE - Multi-room with segments, vacuum at dock",
            ),
        ]
        ...
        for i, dataset in enumerate(self.test_data_sets):
            ...
            if data:
                ...
                if robot and charger:
                    distance = (
                        (robot[0] - charger[0]) ** 2 + (robot[1] - charger[1]) ** 2
                    ) ** 0.5
                    at_dock = distance < 500  # Within 500 units
                    print(
                        f"   🏠 At dock: {'✅ YES' if at_dock else '❌ NO'} (distance: {distance:.0f})"
                    )
            else:
    ...
    async def test_image_handler(self):
        ...
        for i, dataset in enumerate(self.test_data_sets):
            ...
            background_tasks.append(
                self.simulate_ha_background_task("sensor_updates", 2.0)
            )
            background_tasks.append(
                self.simulate_ha_background_task("automation_engine", 1.5)
            )
            background_tasks.append(
                self.simulate_ha_background_task("state_machine", 1.8)
            )
            background_tasks.append(
                self.simulate_ha_background_task("websocket_handler", 2.2)
            )
            background_tasks.append(self.simulate_ha_background_task("recorder", 1.3))

            _LOGGER.info(
                f"🏠 Starting image generation {i + 1}/{len(self.test_data_sets)} with HA simulation..."
            )
            ...
            try:
                ...
                results = await asyncio.gather(
                    image_task, *background_tasks, return_exceptions=True
                )
                ...
                _LOGGER.info(
                    f"✅ Image {i + 1}/{len(self.test_data_sets)} completed in {end_time - start_time:.3f}s with concurrent load"
                )
    ...
    async def _generate_single_image(self):
        ...
        if self.profiler:
            self.profiler.take_memory_snapshot(
                f"Before Image Gen - {self.current_file}"
            )
            cpu_profiler = self.profiler.start_cpu_profile(
                f"Image Generation - {self.current_file}"
            )
        ...
        shared.rand256_active_zone[4] = (
            0  # Enable Living Room (array index 4, segment ID 16)
        )
        ...
        _LOGGER.info(
            await handler.get_rooms_attributes(
                {
                    "spots": [{"name": "test_point", "coordinates": [25566, 27289]}],
                    "zones": [
                        {
                            "name": "test_zone",
                            "coordinates": [[20809, 25919, 22557, 26582, 1]],
                        }
                    ],
                    "rooms": [
                        {"name": "Bathroom", "id": 19},
        ...
        if robot_room_result and "in_room" in robot_room_result:
            ...
            if handler.robot_in_room and "id" in handler.robot_in_room:
                ...
                if len(handler.shared.rand256_active_zone) > room_id:
                    _LOGGER.info(
                        f"Active zone for room {room_id}: {handler.shared.rand256_active_zone[room_id]}"
                    )
                else:
                    _LOGGER.info(
                        f"Room ID {room_id} is out of range for active zones array (length: {len(handler.shared.rand256_active_zone)})"
                    )
            else:
        ...
        _LOGGER.info(
            f"Zoom conditions: zoom={handler.zooming}, vacuum_state={handler.shared.vacuum_state}, image_auto_zoom={handler.shared.image_auto_zoom}"
        )
        ...
        if self.profiler:
            ...
            self.profiler.time_operation(
                f"Image Generation - {self.current_file}", start_time, end_time
            )
