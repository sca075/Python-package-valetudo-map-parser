...
class HypferRoomsHandler:
    ...
    async def async_moore_neighbor_trace(mask: np.ndarray) -> List[Tuple[int, int]]:
        """
        Trace the contour of a binary mask using Moore-Neighbor tracing.
        ...
        while True:
            ...
            for i in range(8):
                ...
                if padded[ny, nx] == 1 and (nx, ny) not in visited:
                    current = (nx, ny)
                    prev_dir = (dir_idx + 5) % 8
                    found = True
                    break

            if not found or (current == start and len(contour) > 3):
                break
